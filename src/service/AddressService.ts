...
export class AddressServiceImpl implements AddressService {
    ...
    public async getAddresses(): Promise<Array<Address>> {
        ...
        const addressesMongo: Array<Address> = await this.addressRepositoryMongo.findAll().then((a) => a.map((dto: AddressDTO) => {
            return this.transformAddressDTO(dto);
        }));
    ...
    public async createAddress(address: Address): Promise<Address> {
        const addressDTO: AddressDTO = this.transformAddress(address);
        ...
        return await this.transformAddressDTO(createdDTO);
    }
    ...
    public async updateAddress(address: Address): Promise<Address> {
        ...
        return await this.transformAddressDTO(updated);
    }
    ...
    private transformAddress(address: Address): AddressDTO {
        return {
    ...
    private transformAddressDTO(addressDTO: AddressDTO): Address {
        return new Address(
            addressDTO.address1,
            addressDTO.address2,
            addressDTO.city,
